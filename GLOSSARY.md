...
**Node** — a single model (or seed, in these examples) in a dbt project's compiled dependency
graph, as `zhao-dbt-plan` and `zhao-cli` both read it from `manifest.json`.
...
correctly widened window to everything downstream of it.

**Cascading window** — the widened time window `zhao-dbt-plan` computes for each downstream tier
of a microbatch chain, by walking each model's own `lookback`/`lookahead` config outward from
either an Anchor or the default Entry Node window.
...
result. Read by both `zhao-dbt-plan`'s planner and `zhao_dbt_utils`'s macros, from the same
`meta.zhao` block, so the two never need to be kept in sync by hand.
